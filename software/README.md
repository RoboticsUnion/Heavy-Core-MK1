...
Our Software consists of three parts:
    - RoboticsUnion Control UI
    - RoboticsUnion Robot Control
    - RoboticsUnion Firmware

# RoboticsUnion Control UI

The RoboticsUnion Control UI is the highest layer of control in our software. 
It can be run on any Windows PC that is connected to a robot via LAN.

The Conrol UI shows important data from the robot's sensors and is able to transmit data to the robot. This way you can control a connected robot.
...
It also serves as a link between the Firmware and the RoboticsUnion Control UI by translating high-level commands into low-level hardware instructions.
